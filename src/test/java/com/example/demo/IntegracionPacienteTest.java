...
public class IntegracionPacienteTest {
    ...
    public void cargarDataSet() throws BadRequestException {
        ...
        Paciente p2 = pacienteService.save(new Paciente("Santiago", "Paz", 33333333, LocalDate.of(2000, 1, 1), domicilio2));
    }
    @Test
    public void listarPacientes() throws Exception {
        this.cargarDataSet();
        MvcResult response = mockMvc.perform(MockMvcRequestBuilders.get("/pacientes/{id}", 2).accept(MediaType.APPLICATION_JSON))
    ...
    public void eliminarPaciente() throws Exception {
        this.cargarDataSet();
        MvcResult response = mockMvc.perform(MockMvcRequestBuilders.delete("/pacientes/{id}", 1).accept(MediaType.APPLICATION_JSON))
                .andDo(MockMvcResultHandlers.print())
                .andExpect(MockMvcResultMatchers.status().isOk()).andReturn();
